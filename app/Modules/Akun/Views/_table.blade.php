...
<script>
    $('#o-new').on('click',function(e){
        ...
        // console.log('klik')
		loadshow()
        $('#modal').empty().load("{{ route('akun-load') }}",'a=_input',function(){
            $('#modal').modal('show')
			loadhide()
        })
    })
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, full, meta) {
					...
					let role=full.role.split(',')

                    let warna='badge-primary'
					let a=''
					for(let i in role){
						a+=`<span class="badge ${warna} mr-1">${role[i]}</span>`
					}
                    return a
                }
            },
    ...
    .on( 'click', '.list-edit', function (e) {
		...
		loadshow()
        let id=$(this).data('id')
        $('#modal').empty().load("{{ route('akun-load') }}",'a=_edit&id='+$(this).data('id'),function(){
            $('#modal').modal('show')
			loadhide()
        })
    })
